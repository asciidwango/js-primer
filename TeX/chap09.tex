...
JavaScriptは、\textbf{\textgt{文}}（Statement）と\textbf{\textgt{式}}（Expression）から構成されています。
...
\textbf{\textgt{文}}（Statement）を簡潔に述べると、処理する1ステップが1つの文と言えます。
...
次のような、文を\texttt{\{}と\texttt{\}}で囲んだ部分を\textbf{\textgt{ブロック}}と言います。
ブロックには、複数の文が書けます。
...
functionキーワードから文を開始する\textgt{関数宣言}と、変数へ\textgt{関数式}を代入する方法があります。
...
これは、\textgt{ブロックで終わる文}にはセミコロンが不要であるためです。
