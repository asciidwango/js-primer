...
ビルトインオブジェクト\index{びるといんおぶじぇくと@ビルトインオブジェクト}は実行環境に組み込まれたオブジェクトのことです。
...
\texttt{Object.freeze}はオブジェクトを凍結\index{おぶじぇくと@オブジェクト!とうけつ@凍結}\index{とうけつ@凍結}します。凍結されたオブジェクトでプロパティの追加や変更を行うと例外が発生するようになります。

ただし、\texttt{Object.freeze}メソッドを利用する場合は必ずstrict
mode\index{strict mode}と合わせて使います。 strict
...
\texttt{hasOwnProperty}メソッド}{プロパティの存在確認: hasOwnPropertyメソッド}}\label{hasOwnProperty-method}}\index{hasOwnProperty@\texttt{hasOwnProperty}}
