...
多くのプログラミング言語にはコードの評価の仕方として、\textbf{\textgt{同期処理}}（sync）と\textbf{\textgt{非同期処理}}（async）という大きな分類があります。
...
JavaScriptでは一部の例外を除き非同期処理が\textbf{\textgt{並行処理}}（concurrent）として扱われます。
...
このWeb Workerにおける非同期処理は\textbf{\textgt{並列処理}}（Parallel）です。
...
このようにコールバック関数の1番目の引数にはエラーオブジェクトまたは\texttt{null}を入れ、それ以降の引数にデータを渡すというルールを\textbf{\textgt{エラーファーストコールバック}}と呼びます。
...
\hypertarget{promise}{%
\section[Promise]{Promise\protect\icon{fig/es2015.eps}}\label{promise}}
...
これはメソッドチェーンと呼ばれる仕組みですが、この書籍ではPromiseをメソッドチェーンでつなぐことを\textbf{Promise\textgt{チェーン}}と呼びます（詳細は「\hyperlink{array}{配列}」の章の「\hyperlink{method-chain-and-high-order-function}{メソッドチェーンと高階関数}」を参照）。
...
\begin{figure}[h]
\centering
\includegraphics[width=60mm]{./fig/then-rejected-promise.eps}
\end{figure}
...
\section[Async Function]{Async Function\protect\icon{fig/es2017.eps}}\label{async-function}}
