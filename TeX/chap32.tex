...
先ほどのAppの\texttt{mount}メソッドでは、\texttt{submit}イベントのイベントリスナー内で\texttt{event.preventDefault}メソッドを呼び出しています。
...
TodoListModelは\texttt{App}クラスの外からは触る必要がないため、\texttt{\#todoListModel}というPrivateクラスフィールドとして定義しています。
...
TodoListModelの\texttt{onChange}メソッドで登録したリスナー関数は、\texttt{TodoListModel}の状態が更新されたら呼び出されます。

このリスナー関数内ではTodoListModelの\texttt{getTodoItems}メソッドでTodoアイテムを取得しています。
...
また、アイテム数はTodoListModelの\texttt{getTotalCount}メソッドで取得できるため、アイテム数を管理していた\texttt{todoItemCount}という変数は削除できます。
...
直接DOMへ\texttt{appendChild}していた部分をTodoListModelの\texttt{addTodo}メソッドを使ってモデルを更新する処理へ置き換えるだけです。
...
まずは確認のため、\texttt{src/App.js}にてTodoListModelの\texttt{onChange}メソッドで登録したリスナー関数内を書き換え、チェックボックスを表示しています。
...
TodoListModelの\texttt{updateTodo}メソッドは、指定したidと一致するTodoアイテムの完了状態（\texttt{completed}プロパティ）を更新します。
...
\texttt{src/App.js}にて、TodoListModelの\texttt{onChange}メソッドで登録したリスナー関数内を次のように書き換えます。

\texttt{App.js}で\texttt{todoItemElement}の子要素として\texttt{checkbox}というクラス名をつけた\texttt{input}要素を追加します。
この\texttt{input}要素の\texttt{change}イベントが発生したら、TodoListModelの\texttt{updateTodo}メソッドを呼び出すようにします。
チェックがトグルするたびに呼び出されるので、\texttt{completed}には現在の状態を反転（トグル）した値を渡します。

\input{./src/todoapp/update-delete/update-feature/App.js.tex}

TodoListModelの\texttt{updateTodo}メソッド内では\texttt{emitChange}\index{emitChange@\texttt{emitChange}}メソッドによって、\texttt{TodoListModel}の変更が通知されます。
これによってTodoListModelの\texttt{onChange}で登録したイベントリスナーが呼び出され、表示が更新されます。
...
TodoListModelの\texttt{deleteTodo}メソッドは、指定したidと一致するTodoアイテムを削除します。
...
このイベントリスナーの中でTodoListModelの\texttt{deleteTodo}メソッドを呼び、指定したidのTodoアイテムを削除します。

\input{./src/todoapp/update-delete/delete-feature/App.js.tex}

TodoListModelの\texttt{deleteTodo}メソッド内では\texttt{emitChange}メソッドによって、\texttt{TodoListModel}の変更が通知されます。
...
TodoItemViewの\texttt{createElement}メソッドの中身は\texttt{App}クラスでのHTML要素を作成する部分を元にしています。
...
TodoListViewの\texttt{createElement}メソッドは\texttt{TodoItemView}を使ってTodoアイテムのHTML要素を作り、\texttt{todoListElement}へと追加していきます。
このTodoListViewの\texttt{createElement}メソッドも\texttt{onUpdateTodo}と\texttt{onDeleteTodo}のリスナー関数を受け取ります。
...
このときTodoListViewの\texttt{createElement}メソッドには次のようにそれぞれ対応するコールバック関数を渡します。

\begin{itemize}
\item
  \texttt{onUpdateTodo}のコールバック関数ではTodoListModelの\texttt{updateTodo}メソッドを呼ぶ
\item
  \texttt{onDeleteTodo}のコールバック関数ではTodoListModelの\texttt{deleteTodo}メソッドを呼ぶ
\end{itemize}
...
また、Appの\texttt{mount}メソッドでTodoListModelの\texttt{onChange}などにイベントリスナーを登録していますが、そのイベントリスナーを解除していません。
