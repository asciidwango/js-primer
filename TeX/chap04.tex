...
この書籍ではブラウザとして\href{https://www.mozilla.org/ja/firefox/}{Firefox}\index{Firefox}を利用します。
...
開発者ツールの``コンソール''タブを選択すると、コマンドライン（二重山カッコ\texttt{>>}からはじまる欄）に任意のコードを入力して評価できます。
...
ファイルを作成するため\href{https://atom.io/}{Atom}\index{Atom}\footnote{\url{https://atom.io/}}や\href{https://code.visualstudio.com/}{Visual
Studio
...
APIに渡す値の問題から起きる\texttt{TypeError}\index{TypeError@\texttt{TypeError}}や存在しない変数を参照しようとして起きる\texttt{ReferenceError}\index{ReferenceError@\texttt{ReferenceError}}などさまざまな種類があります。
