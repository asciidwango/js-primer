...
配列に格納したそれぞれの値のことを\textbf{要素}、それぞれの要素の位置のことを\textbf{インデックス}（\texttt{index}）と呼びます。
...
配列の作成と要素へのアクセス方法は「\href{../data-type/\#array}{データ型とリテラル}」の章ですでに紹介していますが、
もう一度振り返ってみましょう。
...
作成した配列の要素へインデックスとなる数値を、\texttt{配列[インデックス]}と記述することで、
...
このような、配列の中に隙間があるものを\textbf{疎な配列}と呼びます。
一方、隙間がなくすべてのインデックスに要素がある配列を\textbf{密な配列}と呼びます。
...
\begin{tcolorbox}[title=TypedArray{$^{\rm ES2015}$}]\label{typed-array}
...
\section{配列と分割代入{$^{^rm ES2015}$}}\label{array-destructuring}}
...
\begin{tcolorbox}[title=undefinedの要素と未定義の要素の違い]\label{diff-undefined-and-no-element}
...
指定した要素が配列のどの位置にあるかを知りたい場合、\texttt{Array\#indexOf}メソッドや\texttt{Array\#findIndex}メソッド{$^{\rm ES2015}$}を利用します。
...
より明確に要素自体が欲しいということを表現するには、\texttt{Array\#find}メソッド{$^{\rm ES2015}$}が使えます。
...
そこで、ES2016で導入された\texttt{Array\#includes}メソッド{$^{\rm ES2016}$}を利用します。
...
（「\href{../loop/README.md\#array-some}{ループと反復処理}」の章を参照）。
...
\hypertarget{spread}{%
\section{配列の展開{$^{\rm ES2015}$}}\label{spread}}
...
\hypertarget{flat}{%
\section{配列をフラット化{$^{\rm ES2019}$}}\label{flat}}

\texttt{Array\#flat}メソッド{$^{\rm ES2019}$}を使うことで、多次元配列をフラットな配列に変換できます。
...
次のコードでは、\texttt{array}変数に空の配列を代入することで、\texttt{array}に空の配列を参照させられます。
...
& 変更後の配列のlength\tabularnewline
\href{https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Array/copyWithin}{\texttt{Array.prototype.copyWithin}}{[}ES2015{]}
& 変更後の配列\tabularnewline
\href{https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Array/fill}{\texttt{Array.prototype.fill}}{[}ES2015{]}
& 変更後の配列\tabularnewline
\bottomrule
\end{longtable}
...
非破壊的な配列を扱うライブラリの例として\href{https://github.com/azu/immutable-array-prototype}{immutable-array-prototype}や\href{https://facebook.github.io/immutable-js/}{Immutable.js}などがあります。

\hypertarget{array-iterate}{%
\section{配列を反復処理するメソッド}\label{array-iterate}}

「\href{../loop/README.md}{ループと反復処理}」の章において配列を反復処理する方法を一部解説しましたが、改めて関連するArrayメソッドを見ていきます。
...
\begin{tcolorbox}[title=Array-likeオブジェクト]\label{array-like}

配列のように扱えるが配列ではないオブジェクトのことを、\textbf{Array-likeオブジェクト}と呼びます。
Array-likeオブジェクトとは配列のようにインデックスにアクセスでき、配列のように\texttt{length}プロパティも持っています。しかし、配列のインスタンスではないため、Arrayメソッドは持っていないオブジェクトのことです。

\begin{longtable}[]{@{}lll@{}}
\toprule
機能 & Array-likeオブジェクト & 配列\tabularnewline
\midrule
\endhead
...
Array-likeオブジェクトは配列のようで配列ではないというもどかしさを持つオブジェクトです。\texttt{Array.from}メソッド{$^{\rm ES2015}$}を使うことでArray-likeオブジェクトを配列に変換して扱うことができます。一度配列に変換してしまえばArrayメソッドも利用できます。
...
この書籍でもすべてのメソッドは紹介していないため、詳しくは\href{https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Array}{Arrayについてのドキュメント}も参照してみてください。
