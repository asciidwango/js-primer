...
JavaScriptは動的型付け言語に分類される言語であるため、静的型付け言語のような\textbf{変数の型}はありません。
しかし、文字列、数値、真偽値といった\textbf{値の型}は存在します。
これらの値の型のことを\textbf{データ型}と呼びます。

データ型を大きく分けると、\textbf{プリミティブ型}と\textbf{オブジェクト}の2つに分類されます。
...
残念ながら\texttt{typeof null}が\texttt{"object"}となるのは、歴史的経緯のある仕様のバグ\footnote{JavaScriptが最初にNetscapeで実装された際に\texttt{typeof null === "object"}となるバグがありました。このバグを修正するとすでにこの挙動に依存しているコードが壊れるため、修正が見送られ現在の挙動が仕様となりました。
  詳しくは\url{https://2ality.com/2013/10/typeof-null.html}を参照。
  {[}IEEE 754{]}: https://ja.wikipedia.org/wiki/IEEE\_754 {[}文字列{]}:
  ../string/README.md {[}配列{]}: ../array/README.md {[}オブジェクト{]}:
  ../object/README.md {[}ラッパーオブジェクト{]}:
  ../wrapper-object/README.md}です。
...
数値は大きく分けて\texttt{42}のような整数リテラルと\texttt{3.14159}のような浮動小数点数リテラルがあります。
...
\hypertarget{string}{%
\subsection{文字列（String）}\label{string}}
...
\texttt{\\'}のように\texttt{\\}（バックスラッシュ）を使ってエスケープしなければなりません。
...
\subsubsection{テンプレートリテラル{$^{\rm ES2015}$}}\label{template-literal}}

テンプレートリテラルは
\texttt{`}（バッククォート）で囲んだ範囲を文字列とするリテラルです。
テンプレートリテラルでは、複数行の文字列を改行記号のエスケープシーケンス（\texttt{\\n}）を使わずにそのまま書くことができます。

複数行の文字列も\texttt{`}で囲めば、そのまま書くことができます。
...
テンプレートリテラルも他の文字列リテラルと同様に同じリテラル記号を内包したい場合は、\texttt{\\}を使ってエスケープする必要があります。
...
\hypertarget{object}{%
\subsection{オブジェクトリテラル}\label{object}}
...
\hypertarget{array}{%
\subsection{配列リテラル}\label{array}}

オブジェクトリテラルと並んで、よく使われるリテラルとして配列リテラルがあります。
配列リテラルは\texttt{[}と\texttt{]}で値をカンマ区切りで囲み、その値を持つArrayオブジェクトを作成します。
...
正規表現のパターン内では、\texttt{+}や\texttt{\\}（バックスラッシュ）からはじまる特殊文字が特別な意味を持ちます。

次のコードでは、数字にマッチする特殊文字である\texttt{\\d}を使い、1文字以上の数字にマッチする正規表現をリテラルで表現しています。
