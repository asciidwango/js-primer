...
\texttt{String\#codePointAt}メソッド\protect\icon{fig/es2015.eps}は、文字列の指定インデックスにある文字のCode
Pointの値を返します。
...
一方の\texttt{String.fromCodePoint}メソッド\protect\icon{fig/es2015.eps}は、指定したCode
Pointに対応する文字を返します。
...
\texttt{Array.from}メソッド\protect\icon{fig/es2015.eps}は、引数にiterableなオブジェクトを受け取り、それを元にした新しい配列を返します。
iterableオブジェクトとは\texttt{Symbol.iterator}という特別な名前のメソッドを実装したオブジェクトの総称で、\texttt{for...of}文などで反復処理が可能なオブジェクトです（詳細は「\hyperlink{loop-and-iteration}{ループと反復処理}」の章の「\hyperlink{for-of-statement}{for\ldots{}of文}」を参照）。
