\hypertarget{module}{%
\chapter{ECMAScriptモジュール{$^{\rm ES2015}$}}\label{module}}

ECMAScriptモジュールは\href{../../use-case/todoapp/README.md}{Todoアプリのユースケース}で実際に動かしながら学ぶため、ここでは構文の説明とモジュールのイメージをつかむのが目的です。
...
インポートとエクスポートはそれぞれに \textbf{名前つき} と
\textbf{デフォルト} という2種類の方法があります。
...
\textbf{名前つきエクスポート}は、モジュールごとに複数の変数や関数などをエクスポートできます。
...
\textbf{名前つきインポート}は、指定したモジュールから名前を指定して選択的にインポートできます。
次の例では
...
名前つきエクスポート／インポートには\textbf{エイリアス}の仕組みがあります。
...
\textbf{デフォルトエクスポート}は、モジュールごとに1つしかエクスポートできない特殊なエクスポートです。
...
\textbf{デフォルトインポート}は、指定したモジュールのデフォルトエクスポートに名前をつけてインポートします。
次の例では
...
詳しくは\href{../../use-case/todoapp/README.md}{Todoアプリのユースケース}を参照してください。
