...
たとえば、\texttt{1}という数値は符号つき32ビット整数のビットでは、\texttt{00000000000000000000000000000\newline 001}として表現されます。
わかりやすく4ビットごとに区切ると\texttt{0000\_0000\_0000\_0000\_0000\_0000\_\newline 0000\_0001}のような32ビットの集合となります。
...
符号つき32ビット整数では負の数値は、2の補数形式という形式で表現されます。
...
\item \texttt{0000\_0000\_0000\_0000\_0000\_0000\_0000\_0001}の各ビットを反転すると\texttt{1111\_1111\_1111\_\newline 1111\_1111\_1111\_1111\_1110}となる
...
\subsection{AND演算子（\texttt{\&\&}）\index{ANDえんざんし@AND演算子}\index{&&@\texttt{\&\&}}}\label{and-operator}}
...
暗黙的な型変換によって\texttt{false}に変換されるこれらの値をまとめて\textbf{falsy}\index{falsy}な値と呼びます。
...
\subsection{OR演算子（\texttt{||}）}\index{ORえんざんし@OR演算子}\label{or-operator}}
...
\subsection{NOT演算子（\texttt{!}）\index{NOTえんざんし@NOT演算子}\index{"!@\texttt{"!}}}\label{not-operator}}
...
\section[Nullish coalescing演算子（\texttt{??}）]{Nullish coalescing演算子（\texttt{??}）\index{Nullish coalescingえんざんし@Nullish coalescing演算子}\,\protect\icon{fig/es2020.eps}}\label{nullish-coalescing-operator}}\index{??@\texttt{??}}

Nullish coalescing演算子(\texttt{??})は、左辺の値が\textbf{nullish}であるならば、右辺の評価結果を返します。
\textbf{nullish}\index{nullish}とは、評価結果が\texttt{null}または\texttt{undefined}となる値のことです。
