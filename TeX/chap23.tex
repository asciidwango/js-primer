...
\chapter{Map/Set{$^{\rm ES2015}$}}\label{map-and-set}}
...
\texttt{forEach}メソッドはマップが持つすべての要素を、マップへの挿入順に反復処理します。
...
（詳細は「\href{../object/README.md}{オブジェクト}」の章の「\href{../object/README.md\#confirm-property}{プロパティの存在を確認する}」を参照）。
...
（詳細は「\href{../prototype-object/README.md}{プロトタイプオブジェクト}」の章の「\href{../prototype-object/README.md\#not-inherit-object}{\texttt{Object.prototype}を継承しないオブジェクト}」を参照）。
...
\begin{tcolorbox}[title=キーの等価性とNaN]\label{key-and-nan}
...
この挙動は\href{https://developer.mozilla.org/ja/docs/Web/JavaScript/Equality_comparisons_and_when_to_use_them\#Same-value-zero_equality}{Same-value-zero}アルゴリズムと呼ばれます。
...
\hypertarget{set-new}{%
\section{セットの作成と初期化}\label{set-new}}
