...
UTF-16\index{UTF-16}とは、それぞれの文字を16bitのビット列に変換するエンコード方式です。
...
\texttt{String\#charCodeAt}\index{String\#charCodeAt@\texttt{String\#charCodeAt}}メソッドは、文字列の指定インデックスのCode
Unitを整数として返します。 そのCode
Unitの整数値を\texttt{String\#toString}\index{String\#toString@\texttt{String\#toString}}メソッドでhex値（16進数）にしています。
...
文字列を配列へ分解するには\texttt{String\#split}\index{String\#split@\texttt{String\#split}}メソッドを利用できます。
一方、配列の要素を結合して文字列にするには\texttt{Array\#join}\index{Array\#join@\texttt{Array\#join}}メソッドを利用できます。
...
\texttt{String\#length}\index{String\#length@\texttt{String\#length}}プロパティは文字列の要素数を返します。
文字列の構成要素はCode
...
JavaScriptにおいても、\href{https://www.ecma-international.org/publications/standards/Ecma-402.htm}{ECMA-402}\index{ECMA-402}\footnote{\url{https://www.ecma-international.org/publications/standards/Ecma-402.htm}}というECMAScriptと関連する別の仕様として国際化についての取り決めがされています。
...
文字列からその一部を取り出したい場合には、\texttt{String\#slice}\index{String\#slice@\texttt{String\#slice}}メソッドや\texttt{String\#substring}\index{String\#substring@\texttt{String\#substring}}メソッドが利用できます。
...
\texttt{String\#slice}メソッドは\texttt{String\#indexOf}\index{String\#indexOf@\texttt{String\#indexOf}}メソッドなど位置を取得するものと組み合わせて使うことが多いでしょう。
...
\texttt{String\#indexOf}\index{String\#indexOf@\texttt{String\#indexOf}}メソッドと\texttt{String\#lastIndexOf}\index{String\#lastIndexOf@\texttt{String\#lastIndexOf}}メソッドは、指定した文字列で検索し、その文字列が最初に現れたインデックスを返します。
...
\begin{itemize}
\item
  \texttt{String\#startsWith(検索文字列)}\index{String\#startsWith@\texttt{String\#startsWith}}\,\protect\iconl{fig/es2015.eps}:
  検索文字列が先頭にあるかの真偽値を返す
\item
  \texttt{String\#endsWith(検索文字列)}\index{String\#endsWith@\texttt{String\#endsWith}}\,\protect\iconl{fig/es2015.eps}:
  検索文字列が末尾にあるかの真偽値を返す
\item
  \texttt{String\#includes(検索文字列)}\index{String\#includes@\texttt{String\#includes}}\,\protect\iconl{fig/es2015.eps}:
  検索文字列を含むかの真偽値を返す
\end{itemize}
...
正規表現のパターン内では、次の文字は\textbf{\textgt{特殊文字}}\index{とくしゅもじ@特殊文字}\index{せいきひょうげん@正規表現!とくしゅもじ@特殊文字}と呼ばれ、特別な意味を持ちます。特殊文字として解釈されないように入力する場合には\texttt{\textbackslash}\index{\\@\texttt{\textbackslash}}（バックスラッシュ\index{ばっくすらっしゅ@バックスラッシュ}）でエスケープ\index{えすけーぷ@エスケープ}することが必要です。
...
\texttt{String\#indexOf}\index{String\#indexOf@\texttt{String\#indexOf}}メソッドの正規表現版とも言える\texttt{String\#search}\index{String\#search@\texttt{String\#search}}メソッドがあります。
...
そのため、マッチした文字列を取得する\texttt{RegExp\#exec}\index{RegExp\#exec@\texttt{RegExp\#exec}}メソッドと\texttt{String\#match}\index{String\#match@\texttt{String\#match}}メソッドが用意されています。
これらのメソッドは、正規表現のマッチを文字列の最後まで繰り返す\texttt{g}フラグ\index{gフラグ@\texttt{g}フラグ}（globalの略称）と組み合わせてよく利用されます。
...
\item
  ...
  \item
    マッチしない場合は、\texttt{null}を返す
  \item
    正規表現の\texttt{g}フラグが有効化されているときは、マッチした末尾のインデックスを\texttt{lastIndex}\index{lastIndex@\texttt{lastIndex}}プロパティに記憶する
  \end{itemize}
\end{itemize}

\texttt{String\#match}メソッドは正規表現の\texttt{g}フラグなしのパターンで検索した場合、マッチしたものが見つかった時点で検索が終了します。
このときの\texttt{match}\index{match@\texttt{match}}メソッドの返り値である配列は\texttt{index}\index{index@\texttt{index}}プロパティと\texttt{input}\index{input@\texttt{input}}プロパティが追加された特殊な配列となります。
...
このときの\texttt{exec}\index{exec@\texttt{exec}}メソッドの返り値である配列が\texttt{index}\index{index@\texttt{index}}プロパティと\texttt{input}\index{input@\texttt{input}}プロパティが追加された特殊な配列となるのは、\texttt{String\#match}メソッドと同様です。
...
また、最後にマッチした末尾のインデックスを正規表現オブジェクトの\texttt{lastIndex}\index{lastIndex@\texttt{lastIndex}}プロパティに記憶します。
...
\texttt{String\#match}メソッドと\texttt{RegExp\#exec}メソッドのどちらも正規表現のキャプチャリング\index{きゃぷちゃりんぐ@キャプチャリング}\index{せいきひょうげん@正規表現!きゃぷちゃりんぐ@キャプチャリング}に対応しています。
...
文字列の一部を置換したり削除するには\texttt{String\#replace}\index{String\#replace@\texttt{String\#replace}}メソッドを利用します。
...
strict
mode\index{strict mode}では削除できないプロパティを削除しようとするとエラーが発生します
...
\texttt{replace}\index{replace@\texttt{replace}}メソッドは、\textbf{文字列}から第一引数の\texttt{検索文字列}または正規表現にマッチする部分を、第二引数の\texttt{置換文字列}へ置換します。
...
\texttt{replace}メソッドでは、キャプチャ\index{きゃぷちゃ@キャプチャ}した文字列を利用して複雑な置換処理もできます。
