...
そのEcma International中のTechnical Committee 39（TC39）という技術委員会によって、どのような機能を仕様へ入れるかを議論しています。
...
このミーティングの議こと録もGitHub上の[tc39/tc39-notes][]にて公開されています。
...
また、この仕様はなぜこうなったのかということを知りたいと思ったときに、その機能がどのような経緯で入ったのかを調べる手段を知っておくことは大切です。
特にES2015以降は策定プロセスもGitHubを利用したオープンなものとなり、過去のミーティングログなども探しやすくなっています。

しかしながら、JavaScriptにおいては言語機能に関しては[MDN Web Docs][]という優れたリファレンスサイトなどがあります。
そのため、ECMAScriptの仕様そのものを参照する機会は少ないでしょう。

### 言語が進化しているため

ECMAScriptはLiving Standardであり、毎年新しいバージョンがリリースされています。
これは、言語仕様に新しい機能や修正などが日々行われていることを表しています。
