...
次の例では、 `obj` 変数に代入したオブジェクトをキーにして `WeakMap` に値をセットし、そのあとに `obj` 変数に `null` を代入しています。
実際に観測することはできませんが、JavaScriptエンジンは元々`obj`が参照していた `{}`がどこからも参照されなくなったため、GCによって不要なものとしてメモリから解放します。
そのときに、`WeakMap` は弱い参照でkey(`obj`)を参照しているので、同時にその`key`に紐づく値を`WeakMap`オブジェクトから開放します。
結果として、値としてセットされた `"value"` は `WeakMap` から自動的に削除されます。
