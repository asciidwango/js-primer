...
[弱い参照][]とは、ガベージコレクタ（GC）によるオブジェクトの解放を妨げないための特殊な参照です。
...
コードでは弱い参照の振る舞いは観測できないので、JavaScriptエンジン的な視点から説明します。
次の例では、`obj`変数をキーにして値をセットし、そのあと`obj`変数に`null`を代入しています。
JavaScriptエンジンがGCによってメモリから開放できるのは、どこからも参照されていないものだけです。
もともと`obj`変数が参照していた`{}`を参照しているのは、`WeakMap`からの弱い参照だけなので、GCは`{}`をメモリから解放できます。
`WeakMap` でも同じように、どこからも参照されていないキーがあると、そのキーに紐づく値を `WeakMap` から削除できます。
ただし、どのタイミングで実際にメモリから開放するかは、JavaScriptエンジンの実装に依存します。
