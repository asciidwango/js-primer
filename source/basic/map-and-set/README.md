...
GCがメモリから解放できるオブジェクトは、どこからも参照されていないものだけです。
このとき、オブジェクトへの弱い参照があったとしても、ガベージコレクタはそのオブジェクトを解放できます。
...
そのため`{}`は不要になった値としてGCがメモリから解放できます。
また、`WeakMap`は解放されたオブジェクト(`{}`)をキーにして紐づいていた値(`"value"`)を削除できます。
