...
変数宣言には`var`、`let`、`const`が利用できます。`let`が使える場合に`var`を使う理由はひとつもありません。
しかし、既存のコードや既存のライブラリなどでは`var`が利用されている場面もあるため、`var`の動作を理解する必要があります。

`let`は「よりよい`var`」と紹介したように、`let`と`var`を改善する目的で導入された構文です。`const`は再代入できないという点以外は`let`と同じ動作になります。
まずは、`let`と`var`で共通の動作を見ていき、その後に異なる動作を見ていくことにします。

まず最初に、`let`と`var`で共通する動作を見ていきます。
`let`と`var`どちらも、初期値を指定せずに宣言した変数は暗黙的に`undefined`という値になります。
次のコードでは、それぞれ初期値を持たない変数を**宣言した後**に参照すると変数の評価結果は`undefined`となっています。
...
次に、`let`と`var`で異なる動作についてです。

`let`では、変数を**宣言する前**にその変数を参照する`ReferenceError`となります。
...
このように`var`で宣言された変数が宣言前に参照できその値が`undefined`となる特殊な動きをしていることが分かります。
...
つまり、先ほどのコードは実際の実行時には次のように解釈されて実行されていると考えられます。

```js
// varの変数宣言はスコープの先頭に**宣言**部分が巻き上げられる
var x;
console.log(x); // => undefined
// 変数への代入はそのままの位置に残る
x = "varのx";
...
そのため、次のようにブロック`{}`で`var`による変数宣言を囲んでも、`fn`関数直下に**宣言**部分が巻き上げられます。

```js
function fn() {
    // varの変数宣言はブロックスコープを無視してもっとも近い関数スコープに紐づく
    console.log(x); // => undefined
    {
        var x = "varのx";
    }
...
このように、`var`で宣言した変数は巻き上げされてしまうため、宣言より前に参照でき、そしてブロックスコープを無視します。
そのため、変数宣言に`var`のみを利用した場合には、ブロックスコープはないのも同然です。
...
// 巻き上げにより内側のスコープで宣言された変数`x`を参照できる
console.log(x); // => 1
```

`let`や`const`は、`var`の直感的ではない動作を改善するためにES2015で導入されました。
`let`や`const`ではブロックスコープが機能し、`var`は既存のコードの互換性のためにブロックスコープを無視する仕様となりました。

```js
if (true) {
    const x = 1;
}
// ブロックスコープがあるため、内側のスコープに宣言された変数は参照できない
console.log(x); // => ReferenceError: x is not defined
```

<!-- textlint-enable eslint -->

このように`let`、`const`に対して`var`は異なった動作をしていますが、`var`で実現できることは`let`でも実現できます。
そのため、既存のコードを理解するために`var`の知識は必要ですが、`let`が利用できる場合に`var`を利用する理由はありません。
