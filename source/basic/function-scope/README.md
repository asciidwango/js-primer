...
このように内側のスコープで同じ名前を定義することで外側の変数を参照できなくすることを**変数の隠蔽**（shadowing）と呼びます。
...
つまり、先ほどのコードは実行時には、次のように解釈されて実行されていると考えられます。
...
`function`から始まってしまうとJavaScriptエンジンが**関数宣言**と解釈してしまうため、無害な括弧などで囲み**関数式**として解釈させるのが特徴的な記法です。これは次のように書いた場合と意味は同じですが、匿名関数を定義して実行するため短く書くことができ、余計な関数定義がグローバルスコープに残りません。
...
結論からいえば、`printX`関数内にある識別子`x`はグローバルスコープ（＊1）の変数`x`を参照します。
...
次のコードは、動的スコープの動きを説明する**擬似的な言語のコード例**です。
...
- 内側から外側のスコープへと順番に変数が定義されているか探す仕組みをスコープチェーンと呼ぶ
- `var`キーワードでの変数宣言は`function`での関数宣言は巻き上げが発生する
