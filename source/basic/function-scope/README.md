...
この時、現在のスコープ（変数を参照する式が書かれているスコープ）に参照する変数が定義されていない場合、ひとつ外側のスコープに定義されていることを順番に確認します。つまり先ほどの例では次のようなステップで参照したい変数を探索しています。

1. INNERブロックスコープに変数`x`があるかを確認 => ない
2. ひとつ外側のOUTERブロックスコープに変数`x`があるかを確認 => ある

一方、現在のスコープも含め外側のスコープに該当する変数が定義されていない場合は、`ReferenceError`の例外が発生します。
...
この時も、現在のスコープ（変数を参照する式が書かれているスコープ）から外側のスコープへと順番に変数が定義されているかを確認します。この時、一番外側のスコープにも変数が定義されていない場合は、参照できる変数は存在しないため、`ReferenceError`の例外が発生します。つまり先ほどの例では次のようなステップで参照したい変数を探索しています。
...
スコープチェーンの仕組みより現在のスコープに定義されている変数`x`が優先的に参照されます。
