...
一方、`fn`関数の外側から変数`x`は参照できないため`ReferenceError`が投げられます。
...
JavaScriptではガベージコレクションがあるため、手動でメモリから解放するコードを書く必要がありません。
...
つまり、関数が状態をもつ方法として、次のように直接関数に値を代入するという手段をとることができます。
