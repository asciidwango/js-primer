...
今までコードをプログラム直下に書いていましたが、ここにも暗黙的な**グローバルスコープ**と呼ばれるスコープが存在します。
グローバルスコープ（大域スコープ）とは名前のとおりもっとも外側にあるスコープで、プログラム実行時に暗黙的に作成されます。
...
グローバルスコープに定義した変数はグローバル変数と呼ばれ、グローバル変数はあらゆるスコープから参照できる変数となります。
なぜなら、スコープチェーンの仕組みにより内側のスコープにない場合は最終的にグローバルスコープに定義されている変数を参照するためです。
...
ビルトインオブジェクトは実行環境（ブラウザやNode.jsなど）がそれぞれ独自に定義したものも多く存在します。つまり、ビルトインオブジェクトと名前の被らない変数名を避けることは難しいです。そのため、関数などを活用し小さなスコープを中心にしてプログラムを書くことで、ビルトインオブジェクトと同じ名前の変数があっても影響範囲が限定的な状態にすることが望ましいです。
