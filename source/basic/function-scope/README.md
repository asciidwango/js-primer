...
`{`と`}`で囲んだブロックもスコープを作成します。（「[文と式][]」の章を参照）
ブロック内で宣言された変数は、スコープ内からのみ参照でき、スコープの外側からは参照できません。
...
このブロックによるスコープのことを**ブロックスコープ**と呼びます。
ブロックスコープは、`let`や`const`定義した変数はスコープの中でのみ参照できる点は関数スコープと同様です。
（例外的に`var`とfunction宣言はブロックスコープを無視しますが、詳細は後述します）

単独のブロックだけではなく、if文やfor文などもブロックスコープを持っています。
...
for文は、ループごとに新しいブロックスコープを作成する点以外はif文と同じです。
ループごとにブロックスコープが作成されているかは、各スコープには同じ識別子の変数は1つしか宣言できないというルールを考えてみると分かりやすいです。
次のコードでは、`const`で`element`変数を定義していますが、エラーなく定義できます。
これは、ループ毎に別々のブロックスコープが作られているため、「各スコープには同じ識別子の変数は1つしか宣言できない」というルールに反していないためです。
