...
この時、現在のスコープ（変数を参照する式が書かれているスコープ）から外側のスコープへと順番に変数が定義されているかを確認します。この時、内側のINNERブロックスコープには変数`x`はありませんが外側のOUTERブロックスコープに変数`x`が定義されているため参照できます。つまり次のようなステップで参照したい変数を探索しています。
...
この時も、現在のスコープ（変数を参照する式が書かれているスコープ）から外側のスコープへと順番に変数が定義されているかを確認します。この時、一番外側のスコープにも変数が定義されていない場合は、参照できる変数は存在しないため、`ReferenceError`の例外が発生します。つまり次のようなステップで参照したい変数を探索しています。
