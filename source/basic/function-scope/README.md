...
次のコードでは関数の中に関数が定義されています。
このとき`outer`関数のスコープの中に`inner`関数のスコープが定義されています。
...
次のコードでは、内側のスコープから外側のスコープに定義されている`outer_y`変数を参照できています。
...
```js
{
    ...
    const outer_y = "y";
    {
        // 内側のスコープ
        const inner_x = "x";
        // 内側のスコープから外側のスコープへアクセスできる
        console.log(outer_y); // => "y"
    }
    // 同じスコープ内にある変数はアクセスできる
    console.log(outer_y); // => "y"
}
```

JavaScriptでは、現在のスコープ（変数を参照する式が書かれているスコープ）に参照する変数が定義されていない場合、ひとつ外側のスコープに定義されていることを順番に確認します。つまり次のようにステップで参照したい変数を探索します。
...
2. ひとつ外側のスコープに移動
    - 外側にスコープがない場合は3へ
4. どのスコープにも変数`x`がない場合は`ReferenceError`が発生する

この内側から順番に外側のスコープへ順番に変数を探す仕組みのことを**スコープチェーン**と呼びます。

先ほどの例では、内側のスコープには該当する変数がなかったため、外側のスコープに定義された変数が参照されるものでした。
内側と外側のスコープ両方に同じ名前の変数が定義されている場合はどうなるでしょうか？
次のコードでは、内側と外側に同じ名前の変数`x`が定義されています。
スコープチェーンの仕組みより現在のスコープに定義されている変数`x`が優先的に参照されることが分かります。
そのため、それぞれのスコープで参照できる変数が異なります。
...
このようにスコープは階層的な構造となっており、その際の変数はスコープチェーンによって解決されています。
