...
クロージャーはさまざまな用途に利用されますが、主に次のような用途で利用されることが多いです。
...
また、関数を返すまたは受け取る関数のことを高階関数と呼びますが、クロージャの性質を使うことで次のようにnより大きいかを判定する関数を作れます。
最初から`gereterThan5`という関数を定義すればよいのですが、FPの文脈では大分。
