...
その後、任意のタイミングでガーベジコレクションによって回収されメモリ上から解放されます。（JavaScriptではガベージコレクションを明示的に呼び出す方法は標準にはありません。そのため、実際に解放されるタイミングはJavaScriptエンジンの実装に依存します。）
...
つまり、関数の実行終了とデータが解放されるかどうかは直接関係ないことが分かります。
そのデータがメモリ上から解放されるかどうかはあくまで、そのデータを参照されているかによって決定されます。
...
- メモリ管理の仕組み: データは参照されなくなったらガーベジコレクションによって解放される
