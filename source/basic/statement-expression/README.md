...
しかし、この無名関数は**式**であり、この処理は変数を宣言する文の一部であることがわかります。
...
多くのプロジェクトでは[Prettier][]などのコードフォーマッターを使用して、セミコロンを含むコードのスタイルを自動的に統一しています。
また、[ESLint][]などのLintツールを使うことで、セミコロンが必要な箇所を静的に検出することもできます。
