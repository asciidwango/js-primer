...
単純に実行するだけなら、ブロック文がなくても同じ結果になるためです。
...
REPLでは、次のように同じ変数名を再定義すると、構文エラー（`SyntaxError`）が発生します(詳細は「[変数と宣言][]」の章の「[`var`の問題][]」を参照)。
そのため、ブラウザでページをリロードして変数定義をリセットしないといけませんでした。
...
undfined // ここで`count`はもう参照できなくなる
» {
    const count = 1;
}
undfined // ここで`count`はもう参照できなくなる
