...
開発者ツールの"コンソール"タブを選択すると、コマンドライン（二重山カッコ`»`から始まる欄）に任意のコードを入力し評価できます。
...
既に何度も登場していますが、コード内のコメントで`// => 評価結果`と書いている場合があります。
...
JavaScriptにおいては多くのエラーは既に類似するケースがウェブ上に報告されています。
