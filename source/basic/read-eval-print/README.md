...
そのため1行ずつ実行しても、同じ変数名を定義すると構文エラー（`SyntaxError`）となります。
...
JavaScriptの多くの実行環境では、Console APIが**コンソール表示**をするAPIとなっています。
...
| メッセージ | 意味 |
| -- | -- |
| `ReferenceError: x is not defined` | エラーの種類は`ReferenceError`で、`x`という未定義の識別子を参照した |
...
そのため、次のように参照する変数を`value`に変更すれば、このエラーは修正できます。
...
しかし、JavaScriptはとても良く使われている言語なので、ウェブ上には類似するエラーを報告している人も多いです。
