...
`var`はもっとも古くからある変数宣言のキーワードですが、意図しない動作となりやすい問題が知られています。
...
次のコードでは`const`で宣言した変数`bookTitle`に対して値を再代入しているため、`TypeError`というエラーが発生し動作しません。
...
どの変数宣言においても宣言できる変数に利用できる名前のルールは同じです。
