...
この`blockTime`関数を呼び出すと、指定時間が経過するまで次の処理（）は呼ばれません。
...
しかし、`setTimeout`関数を使い、非同期のタスクは1秒後に実行するようにタイマーへ登録しています。
...
次のコードを実行すると`setTimeout`関数で登録したコールバック関数は、タイマーに登録した時間（1秒後）よりも大きく遅れて呼び出されます。
...
const startTime = Date.now();
taskA();
// 1000ミリ秒後(1秒後)にコールバック関数を呼び出すようにタイマーに登録する
setTimeout(() => {
...
多くの環境では、このときの非同期処理のコールバックが呼ばれるまでは2秒以上かかります。
これは、1秒後に非同期のタスクを実行するようにタイマーへ登録自体はできていますが、同期的なブロックする処理によって実行が後ろにずれ込んでしまうためです。
