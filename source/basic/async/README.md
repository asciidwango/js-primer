...
そのため、非同期処理を扱うためのPromiseというビルトインオブジェクト、さらにはAsync Functionと呼ばれる構文的なサポートがあります。
...
Async FunctionはPromiseの上に成り立っている構文です。そのため、どちらも理解することが重要です。

## [ES2015] Promise {#promise}

[Promise][]はES2015で導入された非同期処理の状態を表現するビルトインオブジェクトです。

次のコードの、非同期処理（`asyncPromiseTask`関数）は`Promise`インスタンスを返す非同期処理の例です。
...
それぞれの`Promise`インスタンスが非同期処理の成功や失敗といった状態を持っています。
この`Promise`インスタンスに`then`や`catch`メソッドで成功や失敗といった状態が変化した際に呼び出されるコールバック関数を登録することで、非同期処理の結果に対する処理が書けます。
