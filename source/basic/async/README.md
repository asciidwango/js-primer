...
また、ブラウザやNode.jsなどのAPIには非同期処理のみのものも多いため、非同期処理を避けることはできません。
そのため、JavaScriptには非同期処理を扱うためのPromiseというビルトインオブジェクト、さらにはAsync Functionと呼ばれる構文的なサポートがあります。
...
そのため、非同期処理における例外の扱い方(エラーハンドリング)は極めて重要になります。・
...
[Promise][]はES2015で導入された非同期処理の状態を表現するビルトインオブジェクトです。
また、Promiseには状態変化をした際に呼び出されるコールバック関数を登録できます。
つまり状態を操作?をもつクラスです。

次のコードの`asyncPromiseTask`関数はPromiseを扱う非同期処理を行う例です。

- asyncPromiseTaskは非同期処理の成功失敗を決める
- 受け取る側は成功時の処理と失敗時の処理をコールバック関数で登録する

`asyncPromiseTask`関数は、`Promise`というビルトインオブジェクトのインスタンスを返しています。
`Promise`インスタンスは内部的に非同期処理の成功や失敗といった状態を持っています。
`asyncPromiseTask`関数内では、非同期処理を行い、その結果に応じて`resolve`や`reject`を呼び出すことでPromiseインスタンスの状態を変化されます。
関数の返り値である`Promise`インスタンスに対して`then`や`catch`メソッドで成功や失敗といった状態が変化した際に呼び出されるコールバック関数を登録することで、非同期処理の結果に対する処理が書けます。

```js
// asyncPromiseTask関数はPromiseインスタンスを返す
function asyncPromiseTask() {
    return new Promise((resolve, reject) => {
        // 処理に成功した場合は、resolveする
        // 処理に失敗した場合は、rejectする
    });
}
// asyncPromiseTask関数が成功した時、失敗した時に呼ばれる処理をコールバック関数として登録する
...
非同期処理（`asyncPromiseTask`関数）では、HTTPリクエストをおこなったり、ファイルを読み書きしたりなどするため、常に処理が成功するとは限りません。
`asyncPromiseTask`関数で行った処理結果が成功の場合は、返された`Promise`インスタンスの`then`メソッドで登録したコールバックが呼び出されます。
一方で、`asyncPromiseTask`関数で行った処理結果が失敗の場合は、返された`Promise`インスタンスの`catch`メソッドで登録したコールバックが呼び出されます。

実際に`Promise`インスタンスの扱い方やどのように非同期処理を扱うかを見ていきます。
