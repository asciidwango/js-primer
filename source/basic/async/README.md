...
`Promise`インスタンスは、`asyncPromiseTask`関数内で行った非同期処理が成功したか失敗したかと言う状態を表すオブジェクトです。
`asyncPromiseTask`関数が返した`Promise`インスタンスに対して、`then`や`catch`メソッドで成功時や失敗時に呼び出される処理を登録することで、非同期処理の結果を扱います。
...
そのため、非同期な関数はPromiseという非同期処理の状態や結果をラップしたオブジェクトを返し、その結果が決まったら登録しておいたコールバック関数に結果を渡すという仕組みです。
