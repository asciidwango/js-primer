...
次のコードの`waitSync`関数は指定した`timeout`ミリ秒だけ無限ループを行い処理をブロックする同期的な処理です。
この`waitSync`関数を呼び出すと、指定時間経過するまで次の処理（次の行）が呼ばれません。

{{book.console}}
```js
// 同期的に指定した`timeout`ミリ秒だけ待つ関数
function waitSync(timeout) { 
    // `timeout`ミリ秒経過するまで無限ループを行う
...
console.log("処理を開始");
waitSync(3000); // 3秒間待つ
console.log("次の処理が呼ばれるまで3秒間がブロックされる");
```

このような同期的な重たい処理は、次の処理の実行をブロックするためブラウザでは特に問題となります。
なぜなら、JavaScriptは基本的にブラウザのメインスレッド（UIスレッドとも呼ばれる）で実行されるためです。
そのため、JavaScriptで重たい同期処理を行うと他の処理ができなくなるため、画面がフリーズしたような体感を与えてしまいます。

さきほどの例では3秒間も処理をブロックしているため、3秒間スクロールやクリックなどの他の操作が効かないような影響を引き起こします。
...
コールバック関数はメインスレッドの重たい処理の影響をほとんど受けないはずです。
しかし、実際にはこの非同期処理もメインスレッドの重たい処理の影響を受けます。

そのため、次のコードを実行すると`setTimeout`関数で登録したコールバック関数は、タイマーに登録した時間（10ミリ秒後）より遅れてが呼び出されます。
...
ECMAScriptの仕様では**JobQueue**と呼ばれるキューで次に行うタスクが管理されています。
...
たとえばブラウザでは[Web Worker][] APIを使いメインスレッド以外でJavaScriptを実行できため、非同期処理を**並列（Parallel）**に処理できます。
メインスレッドのJavaScriptとWeb WorkerでのJavaScriptは別々のスレッドで実行されるため、お互いに重たい処理の影響を受けにくくなります。
