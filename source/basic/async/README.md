...
特にブラウザにおいては、重たい同期処理はブラウザの体感への悪影響を与えるためです。
...
このようなにコールバック関数内でエラーをキャッチはできますが、この**非同期処理の中**で例外を発生したかが**非同期処理の外**からは分かりません。
そのため、**非同期処理の中**で例外を発生した場合に、その例外を**非同期処理の外**へ伝える方法が必要です。

JavaScriptでは、この非同期処理における例外の扱い方についてはさまざまなパターンがあります。
