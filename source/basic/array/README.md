...
### `Object.groupBy`メソッド [ES2024] {#array-group-by}
...
しかし、`reduce`メソッドは使い方がやや複雜であるため、別の方法があるなら可能なら避けたほうが読みやすいコードになります。
...
第2引数のコールバック関数が返す値をキーとして、配列の要素をグループ分けしたオブジェクトが作成されます。
