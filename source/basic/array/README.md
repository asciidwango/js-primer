...
配列には複数の要素を格納することができますが、`length`プロパティはその配列の要素数を返します。
...
しかし、ES2015からは`Array#includes`メソッドである要素が含まれているかを判定することができます。
...
もちろん、`var`で宣言していた変数を`const`すると再代入できないためエラーとなります。
