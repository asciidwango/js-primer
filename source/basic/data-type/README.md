...
また、オブジェクトであるため`length`プロパティといったオブジェクトがもつプロパティを参照できます。
...
これは、プロパティ型のデータのプロパティへアクセスする際に、対応するラッパーオブジェクトへ暗黙的に変換してからアクセスするためです。
そのため、ラッパーオブジェクトを明示的に作成するメリットはないので、常にリテラルを使いプリミティブ型のデータを表現することを推奨します。
