...
サロゲートペアを含んだ文字列をそれぞれの**Code Point**へ分解するには、Iteratorを利用するのが簡単です。
...
Code Pointの数を数えた場合も、結合文字など視覚的に見えないものを1つと数えてしまいます。そのため、文字として数えたくないものは正規表現で取り除く必要があるなど、視覚的な**文字列の長さ**を数えるにはさらなる工夫が必要になります。

<!-- textlint-disable preset-ja-technical-writing/no-doubled-joshi -->

ECMAScriptが参照するUnicodeの仕様も更新され続けています。
...
ES2015より前では`String#indexOf`メソッドしか固定文字列の検索ができませんでした。
...
どちらのメソッドも正規表現のパターン内に書かれた`(`と`)`で囲んだ部分を取得できます。
