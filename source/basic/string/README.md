...
文字列にはそのままは入力できないような特殊な文字もあります。
たとえば、先ほどもでてきた改行もそのひとつです。
改行はテンプレートリテラル中にはそのまま入力できますが、`"`（ダブルクオート）と`'`（シングルクオート）の文字列リテラルではそのまま入力できません。
...
エスケープシーケンスとは、`\`と次のような文字を組み合わせることで特殊文字を表現する文字のことです。

次の表では、代表的な[エスケープシーケンス][]を紹介しています。
エスケープシーケンスは、`"`（ダブルクオート）、`'`（シングルクオート）、`` ` ``（バッククオート）どの文字列リテラルの中でも利用できます。
...
このエスケープシーケンスを利用することで、先ほどの`"`（ダブルクオート）の中に改行を入力できます。
...
また、`\`から始まる文字がエスケープシーケンスとして扱われます。
次のように、`\`（バックスラッシュ）そのものを入力していたつもりが、その文字がエスケープシーケンスとして扱われる問題があります。
この場合、`\_`という組み合わせのエスケープシーケンスはないため、`\`が無視された文字列として評価されます。
...
`\`（バックスラッシュ）そのものを入力したい場合は、`\\`のようにエスケープする必要がある点に気をつけてください。
...
- [ ] `>`など`<`での比較も可能だが、これはCode Unitを見てから
...
URLやパスのようにコンテキストをもつ文字列では、文字列操作に気をつける必要があります。
...
そのため、これまでに紹介してきた文字列処理で、サロゲートペアの際に問題がおきる場合がある
...
文字列処理するStringメソッドにはさまざまなものがあり、正規表現との組み合わせ使うものも含まれます。
