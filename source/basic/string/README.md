...
タグ関数の呼び出しに`()`を使っていないことに気をつけてください。

通常の関数のように`()`に引数としてテンプレートリテラルを渡した場合は次のようになります。
...
しかし、`()`を付けずに ``` タグ関数`テンプレート` ``` と記述することで、`タグ関数`が受け取る引数にはタグ付きテンプレート向けの値が渡ってくることに注意してください。
...
`Array#reduce`メソッドを使うことで、テンプレートの文字列と変数を順番に結合できることが分かります。
...
ここで実装した`stringRaw`関数と同様のものが`String.raw`としてネイティブに提供されています。
...
この章では、JavaScriptにおける文字列とは何かやエンコード方式としてUTF-16を採用している点などについて紹介しました。
また、文字列を編集するさまざまな方法や正規表現について簡潔に紹介しました。

正規表現については、正規表現のみでひとつの本が作れるようなJavaScript言語内にある別言語です。
そのため、詳細は[正規表現 - JavaScript | MDN][]などを参照してください。
