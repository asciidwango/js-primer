...
同じ文字列とは、次の条件を満たしているかどうかという判定になります。

- 文字列の要素であるCode Unitが同じ順番で並んでいるか
- 文字列は長さ(length)は同じか

また、`>`や`<`など大小の比較演算子で文字列同士を比較することもできます。
...
また、`ä`（アクセント付きa）と`z`は、通常のアルファベットでは`ä`の方が後ろの位置にありますが、
スウェーデンアルファベットでは`ä`の方が前になります。

このように文字列の並び順を一つとっても、地域や言語によって異なるため、ローカライズする必要があります。
