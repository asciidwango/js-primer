...
また、文字列リテラル中にはユニコードエスケープシーケンスで、直接Code Pointを書くこともできます。
...
直接キーボードから入力が難しい特殊な文字や絵文字などは、ユニコードエスケープシーケンスを使うことでソースコード上に安全に書くことができます。
...
この16ビット1つのことが**符号単位（Code Unit）**であり、文字列における最小の単位で、すべての文字列は最終的にCode Unitとして扱われます。

16ビット1つで表現できる文字の場合は、**Code Point**と**Code Unit**が同じ値を示します。

Code UnitもCode Pointと同じく、ユニコードエスケープシーケンスとして文字列リテラルに書くことができます。
...
具体的にサロゲートペアの文字である「𩸽」は次の2つのCode Unitで表現できます。
