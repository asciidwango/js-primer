...
そのため、「文字列による検索」で登場したメソッドは、特殊文字と`RegExp#test`メソッドで表現できます。
...
`String#replaceAll`メソッドでは、正規表現を使わずにすべてを置換できることにあります。
そのため、検索文字列に正規表現として書くとエスケープが必要な特殊文字もそのまま書いて置換ができます。
