...
テンプレートリテラルは、 `` ` ``（バッククォート）で囲み文字列を作成できる点は、他の文字列リテラルと同じです。
...
一方、配列の要素を結合し文字列にするには`Array#join`メソッドが利用できます。
...
`slice`メソッドについては、既に配列で学んでいますが、基本的な動作は文字列でも同様です。
...
正規表現のパターン内では、次の文字は**特殊文字**と呼ばれ特別な意味をもちます。特殊文字として解釈されないように入力する場合には`\`（バックスラッシュ）でエスケープが必要です。
...
そのため、`RegExp`コンストラクタの引数のパターン文字列では、バックスラッシュから始まる特殊文字は`\`（バックスラッシュ）自体をエスケープする必要があります。
...
つまり、一見すると問題ないように見えますが、実際に動かしてみて初めてわかるような問題が生じやすいです。
...
ECMAScriptの範囲ではありませんが、URLやファイルパスといった典型的なものに対しては既に専用のものがあります。
