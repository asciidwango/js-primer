...
クラスでは、プロトタイプメソッドとインスタンスに対するメソッドの定義ができることを見てきました。
１つのクラスに同じ名前で両方のメソッドを定義した場合はどうなるでしょうか？
...
次のように、インスタンスのメソッド(プロパティ)を`delete`演算子で削除してみると、
プロトタイプのメソッドが呼ばれていることが分かります。
...
この`method`はそれぞれ異なるオブジェクトに対して定義されているため、メソッドの定義自体が上書きされているわけではありません。
プロトタイプメソッドは**プロトタイプオブジェクト**へ、インスタンスのメソッドは**インスタンスオブジェクト**へそれぞれ定義されています。

そして、**プロトタイプオブジェクト**と**インスタンスオブジェクト**のどちらのメソッドを優先して呼びだすかという仕組みは、**プロトタイプチェーン**と呼ばれる仕組みによって動いています。
このセクションでは、このプロトタイプチェーンについてを見ていきます。
