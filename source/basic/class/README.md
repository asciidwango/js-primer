...
`constructor`の中でインスタンスに対するメソッドの定義です。

これらの2つの方法を同時に使い、1つのクラスに同じ名前でメソッドを定義した場合はどうなるでしょうか？

次の`ConflictClass`では`method`という名前のプロトタイプメソッドとインスタンスのメソッドをそれぞれ実装しています。
結論から述べると、このときはインスタンスのメソッドが優先して呼び出されます。
...
インスタンスの`method`プロパティは`delete`演算子で削除できます。
なぜなら、インスタンスオブジェクトの`method`プロパティが関数であるというだけで、プロパティは`delete`演算子で削除できるからです。

次のように、インスタンスの`method`プロパティを`delete`演算子で削除してみると、
今度は"プロトタイプのメソッド"が呼ばれていることが分かります。
...
仮にメソッドが上書きされているならば、`delete`演算子でメソッドを消した時点でインスタンスから呼び出せる`method`はなくなってしまうためです。
つまり、プロトタイプメソッドとインスタンスのメソッドは別々のオブジェクトに定義されたメソッドであることが推測できます。

次のセクションでは、このメソッド定義の違いやメソッド呼び出しの違いの背景にある**プロトタイプ**についてを詳しく見ていきます。

## プロトタイプオブジェクト {#prototype}

先ほどのコードの実行結果からプロトタイプメソッドはインスタンスのメソッドはそれぞれ別々のオブジェクトに定義されているような挙動がわかりました。

実際に、これらのメソッドは異なるオブジェクトに対して定義されています。
プロトタイプメソッドは**プロトタイプオブジェクト**へ、インスタンスのメソッドは**インスタンスオブジェクト**へそれぞれ定義されています。

**プロトタイプタイプオブジェクト**とは、JavaScriptのオブジェクトの`prototype`プロパティに自動的に作成される特殊なオブジェクトです。
クラスも一種の関数オブジェクトであるため、自動的に`prototype`プロパティにプロトタイプオブジェクトが作成されています。

次のコードでは、関数やクラスを定義した際にその関数やクラス自身の`prototype`プロパティにプロトタイプオブジェクトが自動的に作成されていることが分かります。
...
`class`構文のメソッド定義はこのプロタイプオブジェクトに定義されます。
そのためプロトタイプメソッドと呼ばれ、次のように、プロトタイプオブジェクトに定義されていることが確認できます。
また、クラスには`constructor`メソッド（コンストラクタ）が必ず定義され、この`constructor`プロパティはクラス自身を参照します。
...
メソッドがそれぞれ異なるオブジェクトに定義されるため、上書きされずに定義できていることがわかりました。
このプロトタイプオブジェクトは、JavaScriptのクラスにおいて大きな役割を持っています。

次はなぜ、プロトタイプオブジェクトに定義したメソッドがインスタンスから呼び出せるのか、その仕組みついて見ていきます。

### プロトタイプチェーン

`class`構文で定義したプロトタイプメソッドはプロトタイプオブジェクトに定義されます。
そのため、インスタンスオブジェクト自身にはメソッドが定義されていませんが、
インスタンスオブジェクトからプロトタイプメソッドを呼び出すことができます。
...
このインスタンスからプロトタイプメソッドを呼び出せるのは**プロトタイプチェーン**と呼ばれる仕組みよって成り立っています。
プロトタイプチェーンは2つの処理から成り立ちます。

- インスタンス作成時に、インスタンスの`[[Prototype]]`内部プロパティへプロトタイプオブジェクトの参照を保存する処理
- インスタンスのプロパティ（またはメソッド）を参照する時に、`[[Prototype]]`内部プロパティまで探索する処理

### インスタンス作成時に、インスタンスの`[[Prototype]]`内部プロパティへプロトタイプオブジェクトの参照を保存する処理

クラスから`new`演算子によってインスタンスを作成する際に、インスタンスにはクラスのプロトタイプオブジェクトの参照が保存されます。
具体的には、インスタンスオブジェクトの`[[Prototype]]`という内部プロパティにクラスのプロトタイプオブジェクトの参照が保存されます。

`[[Prototype]]`は仕様上定められた内部プロパティであるため通常のプロパティのようにはアクセスできません。
...
- [ ] これはどうせ実行できないので別の表現にする
- newするときに勝手に保存してるんだよーってのがわかりたい

```js
// `new`演算子の擬似的なコード表現
function newInstance(Constructor) {    
    // インスタンスとなるオブジェクトを作成する
    const instance = {};
    // インスタンスの`[[Prototype]]`にプロトタイプオブジェクトを設定する
    Object.setPrototypeOf(instance, Constructor.prototype);
    // `this`をinstanceにしてConstructor.constructorを呼び出す
    Constructor.prototype.constructor.apply(instance, arguments);
    return instance;
}
...
オブジェクトのあるプロパティ名を参照する際に、自身がそのプロパティを持っていない場合でもそこで探索が終わるわけではありません。
オブジェクトの`[[Prototype]]`内部プロパティの参照先にあるプロトタイプオブジェクトに対しても同名のプロパティ名があるかを確認します。
これにより、インスタンス自身が持っていなくても、継承したプロトタイプオブジェクトに定義されたメソッドを呼び出すことができます。
