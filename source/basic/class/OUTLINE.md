...
- [コラム] インスタンスが持つ初期値
    ...
    - メソッドの定義はどこに?
	    ...
	    - メソッドで同じ名前の定義
	    - どちらが優先される => thisへ定義
	    - これはprototypeオブジェクトへの暗黙的な定義の違いです。
	    - これら２つはメソッドを呼び出す側から見ると殆ど同じに見えます。
	    - 明確な違いが分かりやすいthisの例を見てみましょう。
	    - 具体例: constructorのthisとarraow function、メソッドのthisについて
		    - これはconstructorの中でのArrow Functionとメソッド(function)の違いが明確に現れています
	    - 補足: 将来的にはClass field が導入される
		    - ここでは触れない?
