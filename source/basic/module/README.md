...
インポート属性（Import attributes）とは、モジュールをインポートするときに追加の属性情報を指定できる、ES2025で追加された構文です。インポートするモジュールの種別をコード上で明示でき、読み込み方法を制御できます。
...
`import`文はデフォルトでモジュールをJavaScriptとして扱います。`with { type: "json" }`をつけることで、リクエストとレスポンスが一致するかを確認します。一致したときだけJSONとして評価するため、`data.json`がJavaScriptとして評価されることを防ぎます。

インポート属性の`type`は実行環境に依存しますが、ブラウザではCSSやWebAssemblyなどのケースに使われることがあります。
