...
数値から文字列へ明示的に変換する倍は、`String`コンストラクタ関数に値を渡せばよい。
...
「片方が文字列と決まっているなら、もう片方のオペランドは関係なく結果は文字列となるのでは？」と考えるかもしれません。
...
文字列結合演算子を`Symbol()`に対して利用すると例外を投げるようになっています。

```js
"文字列と" + Symbol("シンボルの説明"); // => Error
```

この問題も`String`コンストラクタ関数を使うことで、`Symbol()`を文字列化することで解決できます。
...
後方互換性を維持するために、既存プリミティブ型に対しては残念ながらこのような変更は入れることができませんが、
暗黙的な型変換を避けることが推奨されています。
...
また、`NaN`は何と演算しても結果は`NaN`になる特集な値です。
しかし、 `NaN`はNumber型の一種であるという名前と矛盾したデータといえるかもしれません。
...
また、NaNの特殊な性質として、自分自身と一致しないという特徴があります。
