...
ビット演算子では、オペランドである数値を符号付き32bit整数（`0`と`1`からなる32個のビットの集合）として扱います。

たとえば、`1`という数値は符号付き32bit整数のビットでは、`00000000000000000000000000000001` として表現されます。
わかりやすく4bitごとに区切ると `0000_0000_0000_0000_0000_0000_0000_0001` のような32bitの集合となります。
符号付き32bit整数では、先頭の最上位bitは符号を表し、`0`の場合は正の値、`1`の場合は負の値であることを示しています。

![1の符号付き32bit整数での表現](./img/0000_0000_0000_0000_0000_0000_0000_0001.png)

符号付き32bit整数では負の数値は、2の補数形式という形式で表現されます。
2の補数とは、それぞれのビットを反転して1bitを足した値となります。

たとえば、`1` という数値に対して `-1` という数値を符号付き32bit整数で表現してみます。

- 10進数の`1`は、符号付き32bit整数では`0000_0000_0000_0000_0000_0000_0000_0001`となる
- `0000_0000_0000_0000_0000_0000_0000_0001` の各bitを反転すると `1111_1111_1111_1111_1111_1111_1111_1110` となる
- これに1bitを足すと `1111_1111_1111_1111_1111_1111_1111_1111` となる

これによって、`-1`は符号付き32bit整数では `1111_1111_1111_1111_1111_1111_1111_1111` となります。

![-1の符号付き32bit整数での表現](./img/1111_1111_1111_1111_1111_1111_1111_1111.png)

<!-- textlint-disable ja-technical-writing/sentence-length -->

そのため符号付き32bit整数で表現できる数値の範囲は`-2147483647`（`1111_1111_1111_1111_1111_1111_1111_1111`）から`2147483647`（`0111_1111_1111_1111_1111_1111_1111_1111` ）までとなります。
32bitを超える数値については、32bitをはみ出る最上位のbitが捨てられます。

<!-- textlint-enable ja-technical-writing/sentence-length -->

これから見ていくビット演算子はオペランドを符号付き32bit整数として扱い、その結果を10進数の数値として返します。
...
`15`は、符号付き32bit整数では`0000_0000_0000_0000_0000_0000_0000_1111`となります。
`9`は、符号付き32bit整数では`0000_0000_0000_0000_0000_0000_0000_1001`となります。
これらをAND演算した結果は`0000_0000_0000_0000_0000_0000_0000_1001`となり、ビット演算子は10進数の値である`9`を返します。
...
XOR演算では、オペランドの各ビットがどちらか片方が`1`でもう片方が`0`の場合に`1`となり、それ以外の場合は`0`となります。
...
```js
console.log(~15); // => -16
console.log(~0b0000_0000_0000_0000_0000_0000_0000_1111); // => 1111_1111_1111_1111_1111_1111_1111_0000
```

`x`をビット否定演算子（`~`）で演算した結果は`-(x + 1)`となります。
