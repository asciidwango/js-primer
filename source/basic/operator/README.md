...
また、論理演算子は左から順番に評価されるため、**短絡評価*（ショートサーキット）に利用されます。

### AND演算子（`&&`）

AND演算子（`&&`）は、左辺の値の評価結果が`true`であるならば、右辺の評価結果を返します。
左辺の評価が`true`でないない場合、右辺は評価されません。
このような評価法を**短絡評価**（ショートサーキット）と呼びます。
...
短絡評価は便利ですが、if文が3重4重にネストしているのは不自然なのと同様に、
AND演算子やOR演算子が3つ4つ出てくる場合は、抽象化ができないかを検討するべきサインとなります。
...
falsyとして評価される値として空文字や数値の`0`が含まれている点が挙げられます。

そのため、次のように`prefix`に空文字を指定して、`prefix`を空文字にしたい場合に、
