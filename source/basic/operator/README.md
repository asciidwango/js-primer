...
また、それぞれの二項演算子と代入演算子は組み合わせて利用することができます。
`+=`、`-=`、`*=`、`/=`、`%=`、`<<=`、`>>=`、`>>>=`、`&=`、`^=`、`|=`については、
次のように、演算した結果を代入することができます。
...
代入演算子は1つの変数に値を代入するものでしたが、分割代入を使うことで配列やオブジェクトに値を代入することができます。

分割代入は、代入演算子（`=`）を使うのは同じですが、左辺のオペランドが配列リテラルやオブジェクトリテラルとなります。
次のように、右辺の配列の値を、左辺の配列リテラルの対応する場所へ代入します。
...
同様にオブジェクトも分割代入することができます。
オブジェクトの場合は、右辺のオブジェクトのプロパティ値を、左辺のオブジェクトの対応するプロパティ名へ代入します。
...
そのため、グループ演算子を使い優先順序を明示することができます。
