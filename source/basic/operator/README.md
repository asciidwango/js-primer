...
必要となったタイミングで、あらためてその演算子の動作を見るのがよいでしょう。
...
つまり、プラス演算子（`+`）は数値同士と文字列同士の演算ができます。
...
グループ化演算子は複数の二項演算子が組み合わさった場合に、演算子の優先順序を明示できる演算子です。
...
[演算子の優先順序][]はECMAScript仕様で定義されていますが、演算子の優先度をすべて覚えるのは難しいです。
演算子の優先順序の中でグループ化演算子は優先される演算子となり、グループ化演算子を使い優先順序を明示できます。
...
しかし、ひとつの式で多数の演算を行うよりも、式自体を分けた方が読みやすい場合もあります。
...
それぞれの条件を分解して名前を付ける（変数として定義する）ことも重要です。
