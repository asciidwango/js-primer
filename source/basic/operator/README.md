...
次のコードでは、`+`演算子を使い加算演算を行っています。
このときの`1`と`2`という2つの値がオペランドです。
...
このコードでは`+`演算子に対して、前後に合計2つのオペランドを置いています。
...
同じデータ型のオペランドを比較する場合は、厳密等価演算子（`===`）と同じ結果となります。
...
基本的に等価演算子（`==`）は[暗黙的な型変換][]を行うためバグを引き起こしやすいです。
