...
論理演算子は基本的に真偽値を扱う演算子でOR、AND、NOTを表現できます。
...
このとき、左辺の評価が`true`の場合、オペランドの右辺を評価しません。
次のように、左辺が`true`の場合は、右辺に書いた`console.log`関数自体が実行されません。

{{book.console}}
```js
// 左辺がtrueなので、右辺は評価されない
true || console.log("このログは実行されません"); // 何も出力されない
// 左辺がfalseなので、右辺は評価される
false || console.log("このログは実行されます"); // => "このログは実行されます"
```

このような値が決まった時点でそれ以上評価しないことを**短絡評価**（ショートサーキット）と呼びます。

また、OR演算子は左辺を評価する際に、[暗黙的な型変換][]を行い真偽値として扱います。
JavaScriptでは、[暗黙的な型変換][]ではどの値が`true`でどの値が`false`になるかは、次のルールによって決まります。
...
このオペランドを真偽値に変換してから評価するのはOR、AND、NOT演算子共通の動作です。
...
左辺の評価が`true`ではない場合、右辺は評価しない**短絡評価**（ショートサーキット）の特性を持つのはOR演算子（`||`）と同様です。
...
AND演算子（`&&`）もOR演算子（`||`）と同様に、左辺を評価するには真偽値へと[暗黙的な型変換][]をしてから判定します。
falsyではない場合は、`true`へと変換されます。

{{book.console}}
```js
// 左辺はfalsyではないため、左辺の評価結果を返す
console.log("文字列" && "右辺の値"); // => "文字列"
console.log(42 && "右辺の値"); // => 42
// 左辺がfalsyであるため、右辺の評価結果を返す
console.log("" && "右辺の値"); // => "右辺の値"
console.log(0 && "右辺の値"); // => "右辺の値"
```
...
短絡評価を使うとif文のネストに比べて短く書くことができます。
...
NOT演算子（`!`）もAND演算子（`&&`）とOR演算子（`||`）と同様に真偽値へと[暗黙的な型変換][]を行います。
