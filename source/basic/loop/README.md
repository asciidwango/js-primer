...
また、while文などのような構文ではなく、配列のメソッドを利用して反復処理を行う方法もあります。
これらの配列のメソッドでの反復処理もよく利用するため、合わせてみていきます。
...
このコードは1から10の合計を電卓で計算すればいいので、普通は必要ありません。
...
配列では、`Array`オブジェクトの`some`メソッドで同様のことが行えます。
