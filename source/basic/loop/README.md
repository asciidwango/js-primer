...
## 無限ループ {#inifinite-loop}

反復処理を扱う際に、コードの書き間違いや条件式の判定ミスなどから無限ループを発生させてしまう場合があります。
たとえば、次のコードは条件式が常に`true`となってしまうため、無限ループが発生してしまいます。

<!-- doctest:disable -->
```js
// `i`が`0`以上ならループ処理をするが、`i`は常に0以上なので無限ループする
for (let i = 0; i >= 0; i++) {
    console.log(i);
}
```

無限ループが発生してしまったときは、あわてずにスクリプトを強制的に止めて、コードを修正しましょう。

ほとんどのブラウザは無限ループが発生した際に、自動的に止める機能が含まれています。
また、ブラウザで該当のスクリプトを実行しているページ（タブ）を閉じることでも停止できます。
