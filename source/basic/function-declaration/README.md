...
JavaScriptでは、処理をまとめる際に**関数**と呼ばれる機能を利用します。
関数とはいくつかの文からなる処理を1つの処理としてまとめる機能です。
関数は関数オブジェクトとも呼ばれ、オブジェクトの一種です。
...
関数では`return`文が実行されると、関数ではそれ以降の処理は行われません。
...
`return`文が書かれていない関数は、`undefined`という値を返します。
...
デフォルト引数を使って書くことで、このような挙動は減らすことができ安全です。
...
そのため、引数の違いで関数を分けたい場合は、それぞれを別々の名前で定義するか関数の内部で処理を分岐する必要があります。[^オーバーロード]

## ファーストクラスファンクション {#first-class-function}

関数が値として扱えることをファーストクラスファンクション（第一級関数）と呼びます。
JavaScriptは他のオブジェクトと同じように関数を値として扱えます。
そのため、JavaScriptの関数はファーストクラスファンクションです。

関数を値として定義する場合にも関数宣言と同じく`function`キーワードを利用します。
このとき、関数を式（代入する値）として扱うため**関数式**と呼びます。
関数式には、関数宣言と同じ`function`キーワードを使った定義方法とArrow Functionを使った定義方法があります。
...
名前のとおり矢印のような`=>`（イコールと大なり記号）で匿名関数を定義する構文です。
