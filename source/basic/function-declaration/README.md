...
JavaScriptは関数を他のオブジェクトと同じように値として扱えるため、ファーストクラスファンクションの性質を持っています。
...
// 仮引数の数と定義
const fn0 =     () => { /* 仮引数がないとき*/ };
const fn1 =    (x) => { /* 仮引数が1つのみのとき*/ };
const fn1 =      x => { /* 仮引数が1つのみのときは()を省略可能*/ };
const fn2 = (x, y) => { /* 仮引数が複数の時*/ };
// 値の返し方
// 次の２つの定義は同じ意味となる
const mul1 =     x => { return x * x; } // ブロックの中でreturn
const mul2 =     x => x * x;            // 1行のみの場合はreturnとブロックを省略できる
