...
OR演算子（`||`）を使った場合、次のように`prefix`に空文字を指定した場合にもデフォルト値が入ります。
...
そのため、関数の仮引数の定義部分だけみても、実際に関数の要求する引数がわからないという問題を作りやすいです。
...
関数はただのオブジェクトとは異なり、関数名に`()`を付けることで、関数としてまとめた処理を呼び出すことができます。

一方で、`()`をつけて呼び出されなければ、関数をオブジェクトとして参照できます。
また、関数は他の値と同じように変数へ代入したり、関数の引数として渡すことが可能です。
...
そのため、引数の違いで関数を呼び分けたい場合は、別々の名前で関数を定義するか関数の内部で引数の値を見て処理を分岐する必要があります。[^1]
...
この`obj.method1`プロパティと`obj.method2`プロパティはそれぞれメソッドです。
