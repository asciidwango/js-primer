...
今の`getUserInfo`関数ではloadイベントのコールバック関数でHTMLの組み立てと表示も行っています。
これを`then`を使って段階的に処理を行うように書き換えると、次のコードのようにできます。
`getUserInfo`関数では、`resolve`関数に`userInfo`を渡し、次の`then`ではそれをコールバック関数の引数として受け取っています。
同じように、`userInfo`を受け取った関数は`createView`関数を呼び出し、その戻り値を次の`then`に渡しています。

今回の例では、`userInfo`から`view`のように、戻り値の型が変わる部分を区切りとしてthenチェーンを作りました。
thenチェーンの便利なところは、`then`に渡すコールバック関数の戻り値がPromiseであるときに、そのPromiseの完了を待ってくれるところです。
たとえばもし`userInfo`からHTML文字列を組み立てる処理がもっと複雑になり、非同期処理になったとします。
そのとき、今は文字列を返している`createView`関数が、文字列を返すPromiseを返すようになったとしても、thenチェーンに影響はありません。
逆に、同期処理のままでもコールバック関数が違う型を返してしまうと、thenチェーンは壊れてしまいます。
thenチェーンの分割は、コールバック関数の間でやりとりされるオブジェクトの型を基準に考えると、見通しのよいコードになるでしょう。
