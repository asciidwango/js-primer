...
ファイルパスをオプションではなく引数として受け取るために、`program.args`配列から値を取得します。
...
まずは読み込むためのファイルを作成しましょう。`sample.md`という名前で作業ディレクトリに配置します。
...
コマンドラインツールとして用いる場合においても、try/catch文による例外処理を避けられるため、非同期形式でコールバック関数を使うほうが好まれます。
...
`Buffer`はファイルの中身をバイト列として保持していて、そのまま`console.log`関数に渡しても人間が読める文字列にはなりません。
...
ファイルの読み書きは存在の有無や権限、OSによる違いなどによって例外が発生しやすいので、必ずエラーハンドリング処理を書きましょう。
次の例は`err`オブジェクトが`null`または`undefined`ではないことだけをチェックする、もっともシンプルなエラーハンドリングです。
エラーが発生していたときには標準出力に表示し、`process.exit`関数を使って実行しているプロセスを異常終了させています。

[import read-file-3.js](src/read-file-3.js)

存在しないファイルである`notfound.md`を引数に渡した実行結果は次のようになります。
