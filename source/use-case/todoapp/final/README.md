...
表示から発生したイベントをModelに伝え、Modelから発生した変更イベントを表示に伝えている管理者といえます。
...
このような表示のための処理を部品ごとのモジュールに分け、`App`クラスから作成したモジュールを使うような形にリファクタリングをしていきます。
...
この受け取ったリスナー関数はそれぞれ対応するイベントがViewで発生した際に呼びだします。
...
実はこのTodoアプリはまだアプリケーションとして、完成していない部分があります。
...
余力がある人は、次の残ったTodoアプリを完成させてみてください。

- タイトルが空の場合は、フォームを送信してもTodoアイテムを追加できないようにする
- `App#mount`でイベントリスナーの登録に対応して、`App#unmout`を追加しイベントリスナーを解除する処理を追加する
