...
このように、`npx`コマンドを使うことでnpmで公開されているコマンドラインツールを簡単に実行できます。
...
グローバルインストールされたパッケージのコマンドは、`node`コマンドや`npm`コマンドと同じように、任意の場所で実行できるようになります。

次の例では`@js-primer/hello-world`パッケージがグローバルインストールされ、パッケージに定義されている`js-primer-hello-world`コマンドが実行できるようになります。
