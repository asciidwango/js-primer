...
`@js-primer/local-server`は、デフォルトのポート（3000番ポート）が既に使用されているなら、使われてないポートを探してローカルサーバーを起動します。また、`--port`オプションで任意のポート番号でローカルサーバーを起動できます。
...
npmでは既に多種多様なローカルサーバーモジュールが公開されています。
